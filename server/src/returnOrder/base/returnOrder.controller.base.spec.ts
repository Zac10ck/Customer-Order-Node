...
describe("ReturnOrder", () => {
  ...
  test("GET /returnOrders/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/returnOrders"}/${nonExistingId}`)
      .expect(404)
      .expect({
        statusCode: 404,
  ...
  test("GET /returnOrders/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/returnOrders"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
